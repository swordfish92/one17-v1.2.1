...
// Invariant 48(d): "After execution, the builder performs READ-ONLY
// verification of the resulting state and produces the 48(b) exit
// report." This script performs ONLY reads (findMany/findUnique/count) --
// no create/update/delete anywhere in this file. Run against
// one17_walkthrough via the same session-scoped DATABASE_URL override
// pattern as scripts/run-invariant48-walkthrough.ps1, never against .env's
// default target.
...
async function main() {
  ...
  for (const s of staff) console.log(`  ${s.displayName} <${s.email}> — ${s.roles.map((r) => r.roleCode).join(',')}`);
  ...
  for (const p of products) console.log(`  ${p.code}: status=${p.status}, shariahApprovedAt=${p.shariahApprovedAt?.toISOString().slice(0, 10)}, ref=${p.shariahApprovalRef}`);
  ...
  for (const e of entities) console.log(`  ${e.code} -> productCode=${e.productCode}`);
  ...
  for (const n of navRecords) console.log(`  ${n.valuationDate.toISOString().slice(0, 10)}: NAV/unit ${n.navPerUnit}`);
  ...
  for (const r of subReqs) byStatus[`${r.requestType}:${r.status}`] = (byStatus[`${r.requestType}:${r.status}`] ?? 0) + 1;
  ...
  for (const d of distributions) console.log(`  ${d.productCode} period ${d.periodStart.toISOString().slice(0, 10)}..${d.periodEnd.toISOString().slice(0, 10)}: status=${d.status}`);
  ...
  if (sahel) {
    ...
    for (const a of apps) console.log(`  facility application ${a.id}: status=${a.status}, requestedAmountKobo=${a.requestedAmountKobo}`);
    const memos = await prisma.investmentMemo.findMany({ where: { facilityApplicationId: { in: apps.map((a) => a.id) } } });
    for (const m of memos) console.log(`  investment memo ${m.id}: status=${m.status}`);
  }
  ...
  for (const e of encumbrances) console.log(`  encumbrance "${e.description}": status=${e.status}, amountKobo=${e.amountKobo}`);
  ...
  for (const s of sends) console.log(`  marketing send "${s.subject}": status=${s.status}, recipientCount=${s.recipientCount}`);
  const reports = await prisma.stakeholderReportRun.findMany({ where: { periodStart: new Date('2026-04-01') } });
  for (const r of reports) console.log(`  stakeholder report ${r.reportType}: status=${r.status}`);
  ...
  for (const p of periods) console.log(`  ${p.periodStart.toISOString().slice(0, 10)}..${p.periodEnd.toISOString().slice(0, 10)}: status=${p.status}`);
