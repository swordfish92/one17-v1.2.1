...
async function main() {
    ...
    if (tierKpis.length === 0)
        throw new Error(`No ${orgUnitCode}/${tier} kpi_definition rows seeded — cannot run this smoke test.`);
    ...
    if (Math.round(weightSum) !== 100)
        throw new Error(`${orgUnitCode}/${tier} kpi_weight_matrix rows sum to ${weightSum}%, not 100% — cannot activate (DB CHECK would reject).`);
    ...
    if (cadreEmolument.length === 0)
        throw new Error(`No ${cadre} / notch ${notch} emolument_structure rows seeded — cannot run this smoke test.`);
    ...
    await expectReject('cannot self-score before the cycle is open for SCORING twice / with the wrong user', () => pms.submitSelfScore({ cycleId: cycle.id, employeeId: employee.id, scores: scorecard.map((s) => ({ kpiDefinitionId: s.kpiDefinitionId, selfScorePct: 95 })), actorUserId: outsider.id }));
    ...
    await expectReject('an outsider (no PMS_SUPERVISOR_VALIDATION) cannot adjust a validated score', () => pms.adjustValidatedScore(submission.id, scorecard[0].kpiDefinitionId, 80, outsider.id));
    ...
    if (step3.status === 'APPROVED' && submissionAfter.status === 'VALIDATED') {
        ok(`scoring chain (supervisor -> SAU QA -> Chief, three DIFFERENT capabilities) validates the submission -> VALIDATED`);
    }
    else {
        fail('scoring chain did not complete correctly', { step1: step1.status, step2: step2.status, step3: step3.status, submissionStatus: submissionAfter.status });
    }
    await expectReject('outsider (no PMS_CYCLE_MANAGEMENT) cannot propose a scorecard override', () => pms.proposeScorecardOverride({ employeeId: employee.id, kpiDefinitionId: scorecard[0].kpiDefinitionId, overrideWeightPct: 40, reason: 'smoke test', createdByUserId: outsider.id }));
    ...
    if (cycleApproval.status === 'APPROVED' && cycleAfter.status === 'APPROVED') {
    ...
    if (belowThreshold.gatesApplied === 0)
        ok('runTaskDefaultGateFeed applies no gate when the defaulted-task count is below the lowest configured task_default_gate_policy threshold (1)');
    else
        fail('expected zero gates applied below the lowest threshold', belowThreshold);
    ...
    if (gateCheckCountBefore === 0)
        ok('no behavioural_gate_check exists yet for this employee — computeIncentive would skip it rather than assume PASS');
    else
        fail(`expected zero gate checks before recording one, got ${gateCheckCountBefore}`);
    ...
    await prisma.appraisalCycle.update({ where: { id: cycle.id }, data: { status: 'APPROVED' } });
    ...
    async function postPmsPayroll() {
        await expectReject('the maker (HR_ADMIN) cannot also review their own payroll run (maker != checker)', () => workflowInbox.approve(payrollRun.workflowInstanceId, hrAdmin.id));
        await expectReject('an outsider (no PAYROLL_FINANCE_REVIEW) cannot perform the Finance review step', () => workflowInbox.approve(payrollRun.workflowInstanceId, outsider.id));
        await expectReject('MD_CEO cannot skip ahead and approve before Finance has reviewed (step 1 is PAYROLL_FINANCE_REVIEW, not PAYROLL_CEO_APPROVAL)', () => workflowInbox.approve(payrollRun.workflowInstanceId, md.id));
        ...
        await expectReject('Finance cannot approve their own review step a second time (step 2 now requires PAYROLL_CEO_APPROVAL, which FIN_ADMIN lacks)', () => workflowInbox.approve(payrollRun.workflowInstanceId, finAdmin.id));
    ...
    async function emolumentStructureSection() {
        ...
        await expectReject('outsider (no EMOLUMENT_STRUCTURE_MANAGEMENT) cannot propose an emolument component', () => pms.proposeEmolumentComponent({
            cadre: `SMOKE-CADRE-${RUN}`, notch: 1, component: 'BASIC', componentType: 'SALARY',
            annualAmountKobo: 10000000n, taxable: true, effectiveFrom: new Date(), proposedByUserId: outsider.id,
        }));
        const proposed = await pms.proposeEmolumentComponent({
            cadre: `SMOKE-CADRE-${RUN}`, notch: 1, component: 'BASIC', componentType: 'SALARY',
            annualAmountKobo: 50000000n, taxable: true, effectiveFrom: new Date(), proposedByUserId: finAdmin.id,
        });
        ...
        await expectReject('the SAME FIN_ADMIN who proposed cannot also approve (maker != checker)', () => workflowInbox.approve(proposed.workflowInstance.id, finAdmin.id));
        ...
        const revised = await pms.proposeEmolumentComponent({
            cadre: `SMOKE-CADRE-${RUN}`, notch: 1, component: 'BASIC', componentType: 'SALARY',
            annualAmountKobo: 60000000n, taxable: true, effectiveFrom: new Date(), proposedByUserId: finAdmin.id,
        });
    ...
    }
    {
        ...
        await expectReject('an outsider cannot propose a new KPI definition', () => pms.proposeKpiDefinition({ kraCode: spineKra.code, tier: 'JUNIOR', kpiText: 'Smoke KPI', kpiClass: 'CORE', proposedByUserId: outsider.id }));
        ...
        if (kpiProposal.status === 'DRAFT')
            ok('proposeKpiDefinition creates a DRAFT row, workflow initiated');
        else
            fail('proposeKpiDefinition did not create the expected DRAFT row', kpiProposal);
        await expectReject('a duplicate (kraCode, tier) KPI definition proposal is refused', () => pms.proposeKpiDefinition({ kraCode: spineKra.code, tier: 'JUNIOR', kpiText: 'Different text', kpiClass: 'STRATEGIC', proposedByUserId: hrAdmin.id }));
        await expectReject('the proposer (HR) cannot approve their own KPI definition (maker != checker)', () => pms.approveKpiDefinition({ workflowInstanceId: kpiProposal.workflowInstanceId, approverUserId: hrAdmin.id }));
        const approvedKpi = await pms.approveKpiDefinition({ workflowInstanceId: kpiProposal.workflowInstanceId, approverUserId: md.id });
        if (approvedKpi?.status === 'ACTIVE')
            ok('MD_CEO approves the new KPI definition -> ACTIVE');
        else
            fail('KPI definition approval did not activate as expected', approvedKpi);
        const fourClasses = [
        ...
        await expectReject('a weight matrix proposal not summing to 100% is refused', () => pms.proposeWeightMatrixVersion({ orgUnitCode: spineOrgUnit.code, tier: 'JUNIOR', weights: [{ kpiClass: 'CORE', weightPct: 50 }, { kpiClass: 'STRATEGIC', weightPct: 30 }, { kpiClass: 'SECONDARY', weightPct: 10 }, { kpiClass: 'PROCESS_INTEGRITY', weightPct: 5 }], proposedByUserId: hrAdmin.id }));
        await expectReject('a weight matrix proposal missing a class is refused', () => pms.proposeWeightMatrixVersion({ orgUnitCode: spineOrgUnit.code, tier: 'JUNIOR', weights: [{ kpiClass: 'CORE', weightPct: 60 }, { kpiClass: 'STRATEGIC', weightPct: 40 }], proposedByUserId: hrAdmin.id }));
        ...
        await expectReject('the proposer (HR) cannot approve their own weight-matrix version (maker != checker)', () => pms.approveWeightMatrixVersion({ workflowInstanceId: weightProposal.workflowInstance.id, approverUserId: hrAdmin.id }));
        await expectReject('a second HR_ADMIN (not MD_CEO) cannot approve a weight-matrix version — MD_CEO-only, no exceptions', () => pms.approveWeightMatrixVersion({ workflowInstanceId: weightProposal.workflowInstance.id, approverUserId: finAdmin.id }));
        ...
        await expectReject('a weight-matrix version cannot be approved while its org unit/tier has an in-flight PMS cycle (invariant 51c2: never retroactive)', () => pms.approveWeightMatrixVersion({ workflowInstanceId: weightProposal.workflowInstance.id, approverUserId: md.id }));
...
//# sourceMappingURL=pms.smoke.js.map