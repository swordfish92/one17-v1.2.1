...
async function main() {
    ...
    const lifecycle = new employee_lifecycle_service_1.EmployeeLifecycleService(prisma, audit, delegation, workflow, userDeactivation);
    ...
    await expectReject('outsider cannot propose a new hire', () => lifecycle.requestOnboarding({ surname: 'Smoke', firstName: 'Candidate', positionId: position.id, orgUnitCode: orgUnit.code, hireDate: new Date(), requestedByUserId: outsider.id }));
    ...
    await expectReject('HR cannot approve their own onboarding request', () => lifecycle.decideOnboarding(onboarding.workflowInstanceId, hr.id, 'APPROVE'));
    ...
    await expectReject('HR cannot approve their own offboarding request', () => lifecycle.decideOffboarding(offboarding.workflowInstanceId, hr.id, 'APPROVE'));
    ...
    await expectReject('cannot offboard an employee who is already TERMINATED', () => lifecycle.requestOffboarding(employee.id, 'Duplicate attempt.', hr.id));
    ...
    await expectReject('performanceIncentivePct approval refused outright while a cycle is in flight (invariant 50b addendum: never retroactive)', () => lifecycle.decideIncentivePctChange(secondPctRequest.workflowInstanceId, mdCeo.id, 'APPROVE'));
    ...
    if (rejected.status === 'REJECTED' && !rejected.resultingEmployeeId)
        ok('rejected onboarding creates no Employee row');
    else
    ...
    const wfIds = [onboarding.workflowInstanceId, offboarding.workflowInstanceId, secondHireOnboarding.workflowInstanceId, pctRequest.workflowInstanceId, secondPctRequest.workflowInstanceId, rejectOnboarding.workflowInstanceId].filter((id) => !!id);
    ...
    const userIds = [hr.id, mdCeo.id, outsider.id, employee.appUserId, employeeAfterOffboarding.appUser?.id].filter((id) => !!id);
...
//# sourceMappingURL=employee-lifecycle.smoke.js.map