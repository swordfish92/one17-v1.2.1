...
async function main() {
    ...
    async function postJournal(ledgerEntityCode, journalReference, entryDate, lines, makerId, checkerId) {
    ...
    await expectReject('non-FIN_ADMIN (INVESTOR) cannot open an accounting period', () => period.openPeriod({
        ledgerEntityCode: companyCode,
        periodStart,
        periodEnd,
        openedByUserId: id('investor'),
    }));
    const juneCompanyPeriod = await expectSucceed('FIN_ADMIN opens June 2026 accounting period for the COMPANY entity', () => period.openPeriod({
        ledgerEntityCode: companyCode,
        periodStart,
        periodEnd,
    ...
    const juneJournal = await expectSucceed('post a journal dated inside the OPEN June period — succeeds', () => postJournal(companyCode, `JE-P2-JUNE-${RUN}`, withinPeriodDate, [
        { accountCode: '1000', debitKobo: kobo(1_000) },
        { accountCode: '4010', creditKobo: kobo(1_000) },
    ], id('finadmin'), id('finadmin2')));
    console.log(juneJournal?.status === 'POSTED'
    ...
    if (juneCompanyPeriod) {
        const closeRequest = await expectSucceed('FIN_ADMIN (maker) requests closing the June period', () => period.requestPeriodClose({
            periodId: juneCompanyPeriod.id,
            initiatedByUserId: id('finadmin'),
        }));
        if (closeRequest) {
            await expectReject('maker cannot approve their own period-close request (maker≠checker)', () => period.approvePeriodClose({
                workflowInstanceId: closeRequest.id,
                approverUserId: id('finadmin'),
            }));
            const closed = await expectSucceed('MD_CEO (a different authority) approves the period close', () => period.approvePeriodClose({
                workflowInstanceId: closeRequest.id,
    ...
    const anotherJuneJournal = await expectSucceed('create ANOTHER journal dated inside the now-CLOSED June period (DRAFT succeeds — only posting is gated)', () => ledger.createJournalEntry({
        ledgerEntityCode: companyCode,
    ...
    if (anotherJuneJournal) {
        const lateRequest = await expectSucceed('request posting for the late journal — the request itself succeeds (balance check only)', () => ledger.requestJournalPosting({
            journalEntryId: anotherJuneJournal.id,
            initiatedByUserId: id('finadmin'),
        }));
        if (lateRequest) {
            await expectReject('CHECK2 #1: approving posting into a CLOSED period is rejected (service layer)', () => ledger.approveJournalPosting({
    ...
    if (anotherJuneJournal) {
        ...
        if (reloaded.postingWorkflowInstanceId) {
            await workflow.approveNextStep(reloaded.postingWorkflowInstanceId, id('finadmin2'));
            await expectReject('CHECK2 #1: DB trigger rejects posting into a CLOSED period (service bypassed)', () => prisma.journalEntry.update({
                where: { id: anotherJuneJournal.id },
                data: { status: 'POSTED', postedAt: new Date() },
            }));
        }
    }
    const unapprovedJournal = await expectSucceed('create a fresh DRAFT journal, never submitted for posting', () => ledger.createJournalEntry({
        ledgerEntityCode: companyCode,
    ...
    if (unapprovedJournal) {
        await expectReject('CHECK2 #2: DB trigger rejects POSTED with no posting_workflow_instance_id at all (service bypassed)', () => prisma.journalEntry.update({
        ...
        await expectReject('CHECK2 #2: DB trigger rejects POSTED while the linked workflow is still PENDING_APPROVAL, not APPROVED (service bypassed)', () => prisma.journalEntry.update({
            where: { id: unapprovedJournal.id },
            data: { status: 'POSTED', postedAt: new Date() },
        }));
        await workflow.reject(pendingRequest.id, id('finadmin2'), 'smoke test cleanup');
    }
    await expectReject('CHECK2 #3: a single journal cannot reference accounts from two different ledger entities (service layer) — mirroring must be two separate journals', () => ledger.createJournalEntry({
        ledgerEntityCode: companyCode,
        journalReference: `JE-P2-BADMIRROR-${RUN}`,
        entryDate: new Date(),
        description: 'illegally attempting to span company + fund in one journal',
        createdByUserId: id('finadmin'),
        lines: [
            { accountCode: '1100', debitKobo: kobo(300) },
            { accountCode: '5010', creditKobo: kobo(300) },
        ],
    }));
    const interEntityRef = `IER-P2-${RUN}`;
    const mirrorResult = await expectSucceed('create the CORRECT inter-entity mirror: two separate journals, one per entity, shared inter_entity_ref', () => ledger.createInterEntityMirroredJournals({
        interEntityRef,
        entryDate: new Date(),
    ...
    if (mirrorResult) {
        console.log(mirrorResult.legA.ledgerEntityCode === fundCode &&
    ...
    const reconciled = await expectSucceed('reconcile the inter_entity_ref — both legs present, amounts equal', () => ledger.reconcileInterEntityRef(interEntityRef));
    console.log(reconciled?.ok === true
    ...
    await ledger
        ...
        .then((j) => prisma.journalEntry.update({
        where: { id: j.id },
        data: { interEntityRef: orphanRef },
    }));
    const orphanReconciled = await expectSucceed('reconcile a deliberately orphaned inter_entity_ref (one leg missing)', () => ledger.reconcileInterEntityRef(orphanRef));
    console.log(orphanReconciled?.ok === false && orphanReconciled.legCount === 1
    ...
    const reconciliationList = await expectSucceed("list inter-entity reconciliations touching the COMPANY entity — includes both this run's clean and orphaned refs", () => ledger.listInterEntityReconciliations(companyCode));
    if (reconciliationList) {
        const cleanRow = reconciliationList.find((r) => r.interEntityRef === interEntityRef);
        console.log(cleanRow?.ok === true && cleanRow.legs.length === 2
        ...
        console.log(orphanRow === undefined
    ...
    const julyPeriod = await expectSucceed('FIN_ADMIN opens a second (July) accounting period for the COMPANY entity', () => period.openPeriod({
        ledgerEntityCode: companyCode,
    ...
    if (julyPeriod) {
        const julyCloseRequest = await expectSucceed('FIN_ADMIN requests closing the July period', () => period.requestPeriodClose({ periodId: julyPeriod.id, initiatedByUserId: id('finadmin') }));
        if (julyCloseRequest) {
            const workflowInbox = new workflow_inbox_service_1.WorkflowInboxService(prisma, workflow, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, ledger, {}, {}, {}, period, {}, {}, {}, {}, {}, {}, {}, {}, {});
            ...
            if (declaredViaInbox?.status === 'CLOSED' && julyAfter.status === 'CLOSED') {
...
main().catch((err) => {
    if (err instanceof ledger_types_1.LedgerLifecycleError ||
        err instanceof period_types_1.PeriodLifecycleError) {
...
//# sourceMappingURL=period.smoke.js.map