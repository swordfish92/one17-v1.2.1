...
async function main() {
    ...
    await expectReject('onboarding without credit-bureau consent is refused (invariant 27b addition)', () => counterparties.onboard({
        legalName: 'No Consent Ltd', counterpartyType: 'CORPORATE', purposeOfFinancing: 'Working capital',
        amountSoughtKobo: 500000000n, expectedSourceOfRepayment: 'Trade receivables', creditBureauConsent: false,
    ...
    await expectReject('the DB trigger blocks a Txn against a counterparty that is not COMPLETE_APPROVED', () => prisma.txn.create({
        data: { txnType: 'PLACEMENT', ledgerEntityCode, counterpartyId: lowInvestee.id, valueDate: new Date(), amountKobo: 1000000n, makerUserId: id('finadmin') },
    }));
    ...
    if (lowCase.accumulativeRiskProfile === 'LOW' && !lowCase.eddRequired)
        ok('8-metric aggregation: all LOW -> accumulativeRiskProfile LOW');
    else
    ...
    await expectReject('an outsider without ONBOARDING_IC_REVIEW cannot approve IC1', () => counterparties.recordIc1Review({ workflowInstanceId: lowInstance.id, checklist: { KYC_COMPLETE: 'PASS' }, pass: true, approverUserId: id('outsider') }));
    ...
    await counterparties.recordRiskReview({ workflowInstanceId: lowInstance.id, periodicReviewFrequency: 'QUARTERLY', riskNotes: 'Standard monitoring.', approvedExposureLimitKobo: 300000000n, approverUserId: id('risk') });
    ...
    if (lowFinal.status === 'APPROVED')
        ok('LOW/MEDIUM_LOW chain (IC1 -> Risk -> IC2) reaches APPROVED, no MD step');
    else
    ...
    if (approvedCp.onboardingStatus === 'COMPLETE_APPROVED' && approvedCp.enterpriseLimitKobo === 300000000n) {
    ...
    await expectReject('an outsider without INVESTMENT_MEMO_THRESHOLD_MANAGEMENT cannot propose a threshold', () => counterparties.proposeInvestmentMemoThreshold(1000000000n, id('outsider')));
    const thresholdProposal = await counterparties.proposeInvestmentMemoThreshold(1000000000n, id('cio'));
    ...
    await expectReject('an outsider without INVESTMENT_MEMO_THRESHOLD_MANAGEMENT cannot approve a threshold', () => counterparties.approveInvestmentMemoThreshold(thresholdProposal.workflowInstance.id, id('outsider')));
    const thresholdActivated = await counterparties.approveInvestmentMemoThreshold(thresholdProposal.workflowInstance.id, id('md'));
    if (thresholdActivated.status === 'ACTIVE')
        ok('MD_CEO approves the threshold — DRAFT flips to ACTIVE (maker CIO != checker MD_CEO)');
    else
        fail('threshold approval did not activate the config', thresholdActivated);
    const activeThreshold = await counterparties.getActiveInvestmentMemoThreshold(id('cio'));
    if (activeThreshold?.thresholdKobo === '1000000000')
        ok('getActiveInvestmentMemoThreshold reads the newly-ACTIVE row live');
    else
        fail('active threshold did not read back correctly', activeThreshold);
    const facilityAppIds = [];
    const notYetApprovedCp = await counterparties.onboard({
        ...
        amountSoughtKobo: 100000000n, expectedSourceOfRepayment: 'Sales', creditBureauConsent: true, onboardedByUserId: id('portmgr'),
    });
    counterpartyIds.push(notYetApprovedCp.id);
    await expectReject('a facility application before onboarding is COMPLETE_APPROVED is refused', () => counterparties.submitFacilityApplication({ counterpartyId: notYetApprovedCp.id, requestedAmountKobo: 1000000n, purpose: 'Working capital top-up' }));
    const facilityApp = await counterparties.submitFacilityApplication({ counterpartyId: lowInvestee.id, requestedAmountKobo: 200000000n, purpose: 'Equipment finance' });
    ...
    await expectReject('an outsider without COUNTERPARTY_ONBOARDING cannot push a facility application into the governance chain', () => counterparties.reviewAndSubmitFacilityApplication(facilityApp.id, id('outsider')));
    await expectReject('a facility application with no investment memo cannot enter the review chain (invariant 36f)', () => counterparties.reviewAndSubmitFacilityApplication(facilityApp.id, id('portmgr')));
    await expectReject('an outsider without COUNTERPARTY_ONBOARDING cannot draft an investment memo', () => counterparties.draftInvestmentMemo({ facilityApplicationId: facilityApp.id, dealSummary: 'x', structureType: 'x', amountKobo: 1n, tenorMonths: 1, targetReturnPct: 1, riskNotes: 'x', shariahNotes: 'x', collateralNotes: 'x' }, id('outsider')));
    ...
    if (memo.version === 1 && memo.status === 'DRAFT')
        ok('investment memo drafted as version 1, DRAFT (invariant 36f)');
    else
        fail('investment memo did not draft correctly', memo);
    await expectReject('reviewAndSubmitFacilityApplication still refuses with a DRAFT (not yet CIO-approved) memo', () => counterparties.reviewAndSubmitFacilityApplication(facilityApp.id, id('portmgr')));
    ...
    await expectReject('an outsider without INVESTMENT_MEMO_PM_VALIDATION cannot validate the memo', () => counterparties.approveInvestmentMemo(memoInstance.workflowInstanceId, id('outsider')));
    ...
    await expectReject('an outsider without INVESTMENT_MEMO_CIO_APPROVAL cannot approve the memo', () => counterparties.approveInvestmentMemo(memoInstance.workflowInstanceId, id('outsider')));
    ...
    if (memoApproved.status === 'APPROVED')
        ok('CIO approves the investment memo with a comment, below the governed DoA threshold — invariant 46(c)\'s BELOW_THRESHOLD chain (PM validation then CIO) completes here');
    else
        fail('CIO approval did not reach APPROVED', memoApproved);
    const [memoListed] = await counterparties.listInvestmentMemos(facilityApp.id, id('portmgr'));
    if (memoListed.status === 'CIO_APPROVED')
        ok('the memo now reads CIO_APPROVED off the same row the workflow updated');
    else
        fail('memo status did not flip to CIO_APPROVED', memoListed);
    ...
    await expectReject('disbursing an application that is not yet APPROVED is refused', () => counterparties.markFacilityApplicationDisbursed(facilityApp.id, id('portmgr')));
    await expectReject('an outsider without FACILITY_APPLICATION_RISK_REVIEW cannot approve the Risk step', () => counterparties.approveFacilityApplicationStep(submitted.workflowInstanceId, id('outsider')));
    await expectReject('LEGAL cannot approve a facility-application step (VIEW-only, no approval authority, invariant 27b/36e)', () => counterparties.approveFacilityApplicationStep(submitted.workflowInstanceId, id('legal')));
    ...
    if (financeStageProgress.stageLabel === 'Finance Review')
        ok('after BD approves, the label advances to "Finance Review"');
    else
        fail('progress bar did not advance to Finance review after BD approval', financeStageProgress);
    ...
    if (finalStep.status === 'APPROVED')
        ok('Risk->BD->Finance->Compliance reaches APPROVED — the multi-officer chain invariant 36(e) asks for, each with its own capability and comment');
    else
        fail('facility application multi-officer review chain did not reach APPROVED', finalStep);
    ...
    await expectReject('an outsider without FACILITY_APPLICATION_LEGAL_VIEW cannot list facility applications for Legal', () => counterparties.listFacilityApplicationsForLegalView(id('outsider')));
    ...
    await expectReject('an outsider without FUND_ACCOUNTING_RECEIVABLES cannot record facility terms', () => counterparties.recordFundAccountingTerms(facilityApp.id, 18.5, id('portmgr')));
    ...
    const receivableObligation = await prisma.counterpartyRepaymentObligation.create({
        data: { counterpartyId: lowInvestee.id, facilityApplicationId: facilityApp.id, dueDate: new Date('2026-09-01'), amountKobo: 5000000n, createdByUserId: id('finadmin') },
    });
    ...
    const facilityApp2 = await counterparties.submitFacilityApplication({ counterpartyId: lowInvestee.id, requestedAmountKobo: 50000000n, purpose: 'Inventory financing' });
    ...
    const highDeclined = await counterparties.onboard({
        ...
        amountSoughtKobo: 900000000n, expectedSourceOfRepayment: 'Export proceeds', creditBureauConsent: true,
        onboardedByUserId: id('portmgr'),
    });
    counterpartyIds.push(highDeclined.id);
    const highGrades = [...EIGHT_LOW_GRADES.slice(0, 7), { metricCode: 'SOURCE_OF_WEALTH', grade: 'HIGH', rationale: 'Opaque ownership.' }];
    ...
    if (highCase.accumulativeRiskProfile === 'HIGH')
        ok('a single HIGH metric forces accumulativeRiskProfile HIGH (counterparty template, same rule)');
    else
        fail('single-HIGH-metric rule failed', highCase);
    ...
    if (mdDeclined.status === 'REJECTED')
        ok('MD DECLINED stops the HIGH_RISK chain before IC2');
    else
    ...
    if (declinedCp.onboardingStatus === 'DECLINED')
        ok('onboardingStatus correctly flips to DECLINED on rejection');
    else
        fail('onboardingStatus did not flip to DECLINED', declinedCp);
    await expectReject('deployment remains blocked for a DECLINED counterparty', () => prisma.txn.create({ data: { txnType: 'PLACEMENT', ledgerEntityCode, counterpartyId: highDeclined.id, valueDate: new Date(), amountKobo: 1000n, makerUserId: id('finadmin') } }));
    const ic1FailCp = await counterparties.onboard({
        ...
        amountSoughtKobo: 100000000n, expectedSourceOfRepayment: 'Refinancing', creditBureauConsent: true, onboardedByUserId: id('portmgr'),
    });
    ...
    if (ic1FailResult.status === 'REJECTED')
        ok('IC1 FAIL rejects the counterparty chain immediately — Risk/IC2 never reached');
    else
        fail('IC1 FAIL did not reject the workflow', ic1FailResult);
    ...
    const facilityApp3 = await counterparties.submitFacilityApplication({ counterpartyId: lowInvestee.id, requestedAmountKobo: 40000000n, purpose: 'Working capital' });
    ...
    await expectReject('reviewAndSubmitFacilityApplication refuses with a CIO-approved memo but an incomplete document checklist (invariant 44e)', () => counterparties.reviewAndSubmitFacilityApplication(facilityApp3.id, id('portmgr')));
    await expectReject('a DIFFERENT counterparty cannot upload a document onto this application (ownership check)', () => counterparties.uploadFacilityApplicationDocument({ applicationId: facilityApp3.id, documentType: SMOKE_DOC_TYPE, fileReference: 'smoke://intruder' }, highDeclined.id));
    ...
    if (submitted3.status === 'SUBMITTED')
        ok('reviewAndSubmitFacilityApplication now succeeds once the required-document checklist is complete');
    else
        fail('facility application did not reach SUBMITTED after checklist completion', submitted3);
    await expectReject('documents are only accepted while the application is DRAFT — SUBMITTED refuses further uploads', () => counterparties.uploadFacilityApplicationDocument({ applicationId: facilityApp3.id, documentType: SMOKE_DOC_TYPE, fileReference: 'smoke://too-late' }, lowInvestee.id));
    ...
    const facilityApp4 = await counterparties.submitFacilityApplication({ counterpartyId: lowInvestee.id, requestedAmountKobo: 2000000000n, purpose: 'Large syndicated facility' });
    ...
    if (memo4Listed.status === 'SUBMITTED')
        ok('a memo at/above the governed DoA threshold stays SUBMITTED after CIO approval — IC and MD still pending (invariant 46c)');
    else
        fail('memo status flipped to CIO_APPROVED before IC/MD steps ran', memo4Listed);
    await expectReject('the generic approveInvestmentMemo refuses to process the Investment Committee step (icResolutionRef required)', () => counterparties.approveInvestmentMemo(memo4Instance.workflowInstanceId, id('invcommittee')));
    await expectReject('approveInvestmentMemoAsCommittee refuses a blank icResolutionRef', () => counterparties.approveInvestmentMemoAsCommittee(memo4Instance.workflowInstanceId, id('invcommittee'), ''));
    await expectReject('an outsider without INVESTMENT_COMMITTEE_APPROVAL cannot record the IC resolution', () => counterparties.approveInvestmentMemoAsCommittee(memo4Instance.workflowInstanceId, id('outsider'), 'IC-RES-0001'));
    ...
    await expectReject('approveInvestmentMemoAsCommittee refuses to process the MD step (not the IC step)', () => counterparties.approveInvestmentMemoAsCommittee(memo4Instance.workflowInstanceId, id('md'), 'IC-RES-WRONG-STEP'));
    ...
    if (memo4MdApproved.status === 'APPROVED')
        ok('MD/CEO closes the AT_OR_ABOVE_THRESHOLD chain — invariant 46(c)');
    else
        fail('MD approval did not reach APPROVED', memo4MdApproved);
...
//# sourceMappingURL=counterparty.smoke.js.map