...
async function main() {
    ...
    const marketing = {};
    const workflowInbox = new workflow_inbox_service_1.WorkflowInboxService(prisma, workflow, investors, {}, wm, pms, marketing, {}, {}, {}, {}, {}, {}, zakat, ledger, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {});
    ...
    await expectReject('outsider (no ZAKAT_ADVISORY capability) cannot update the nisab config', () => zakat.updateNisabConfig(85, 1500000n, outsider.id));
    const nisabUpdated = await zakat.updateNisabConfig(85, 1500000n, advisor1.id);
    ...
    await expectReject('rate-calendar pairing enforced: cannot create an assessment before a rate basis is elected', () => zakat.createAssessmentSandbox({ investorId: investorA.investorId, assessmentDateGregorian: new Date('2026-07-06'), assessmentDateHijri: '21 Muharram 1448', createdByUserId: advisor1.id }));
    ...
    await expectReject('outsider cannot declare a Zakat schedule item', () => zakat.declareScheduleItem({ zakatAssessmentRunId: runA.id, scheduleType: 'CASH_BANK', description: 'x', amountKobo: 1n, zakatability: 'ZAKATABLE', declaredByUserId: outsider.id }));
    const { item: cashItem, workflowInstance: cashWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'CASH_BANK', description: 'Cash in hand + bank balances', amountKobo: 676109614n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id,
    });
    ...
    const cashVerified = await workflowInbox.approve(cashWf.id, advisor2.id);
    ...
    const { item: investmentItem, workflowInstance: investmentWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'INVESTMENT', description: 'Cash + agric + real-estate investments', amountKobo: 23399747593n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id,
    });
    const { item: goldItem, workflowInstance: goldWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'GOLD_SILVER', description: 'Jewelry + coins', amountKobo: 50000000n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id,
    });
    const { item: fixedAssetItem, workflowInstance: fixedAssetWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'FIXED_ASSET', description: 'House No. 5 (personal residence)', amountKobo: 30000000000n, zakatability: 'NON_ZAKATABLE', declaredByUserId: advisor1.id,
    });
    const { item: receivableGoodItem, workflowInstance: receivableGoodWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'RECEIVABLE', description: 'Money owed, good receivables', amountKobo: 1700000000n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id,
    });
    const { item: receivableDoubtfulItem, workflowInstance: receivableDoubtfulWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'RECEIVABLE', description: 'Doubtful receivables — zakatable only when received', amountKobo: 4420000000n, zakatability: 'DOUBTFUL', declaredByUserId: advisor1.id,
    });
    const { item: liabilityItem, workflowInstance: liabilityWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'LIABILITY', description: 'Short-term payables due within 1 year', amountKobo: 2150000000n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id,
    });
    const { item: longTermLiabilityItem, workflowInstance: longTermLiabilityWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runA.id, scheduleType: 'LIABILITY', description: 'Mortgage / long-term debt (never deducted)', amountKobo: 6000000000n, zakatability: 'NON_ZAKATABLE', declaredByUserId: advisor1.id,
    });
    ...
    if (agreed.status === 'CLIENT_AGREED')
        ok(`client A agrees to proceed (DRAFT -> CLIENT_AGREED)`);
    else
        fail(`expected CLIENT_AGREED`, agreed);
    const computed = await zakat.computeAssessment(runA.id);
    const expectedNetKobo = 676109614n + 23399747593n + 50000000n + 1700000000n - 2150000000n;
    ...
    const expectedDueKobo = (expectedNetKobo * 2577n) / 100000n;
    ...
    if (runAfterSubmit.status === 'AWAITING_SHARIAH_CERTIFICATION')
        ok(`assessment submitted for Shariah certification (CLIENT_AGREED -> AWAITING_SHARIAH_CERTIFICATION)`);
    else
        fail(`expected AWAITING_SHARIAH_CERTIFICATION`, runAfterSubmit);
    ...
    const certified = await workflowInbox.approve(certWf.id, shariahReviewer.id);
    ...
    await expectReject('a CERTIFIED assessment is immutable — no further schedule items may be declared', () => zakat.declareScheduleItem({ zakatAssessmentRunId: runA.id, scheduleType: 'CASH_BANK', description: 'late addition', amountKobo: 1n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id }));
    ...
    await expectReject('invariant 71(a)(i): a CERTIFIED record is unaffected by any later declaration attempt', () => zakat.declareScheduleItem({ zakatAssessmentRunId: runA.id, scheduleType: 'GOLD_SILVER', description: 'another late addition', amountKobo: 99999900n, zakatability: 'ZAKATABLE', declaredByUserId: advisor1.id }));
    ...
    await expectReject('adversarial (v): the Zakat position read model itself refuses before subscription approval (tab invisible, server-computed, not a client-side flag)', () => zakat.getZakatPositionForClient(investorC.investorId));
    ...
    await expectReject('cross-client leak: investor B cannot declare an item on investor C\'s assessment', () => zakat.declareScheduleItemAsClient(investorB.investorId, { zakatAssessmentRunId: runC.id, scheduleType: 'CASH_BANK', description: 'leak attempt', amountKobo: 1n, zakatability: 'ZAKATABLE' }));
    const { item: clientCashItem, workflowInstance: clientCashWf } = await zakat.declareScheduleItemAsClient(investorC.investorId, {
        zakatAssessmentRunId: runC.id, scheduleType: 'CASH_BANK', description: 'Client-declared savings account', amountKobo: 50000000n, zakatability: 'ZAKATABLE',
    });
    ...
    if (agreedC.status === 'CLIENT_AGREED')
        ok(`client C agrees to proceed (DRAFT -> CLIENT_AGREED)`);
    else
        fail(`expected CLIENT_AGREED`, agreedC);
    await expectReject('invariant 71(a)(i): certification is refused while the client-declared item is still non-final (neither VERIFIED nor REJECTED), naming the outstanding item', () => zakat.submitForCertification(runC.id, bd1.id));
    const systemPortalClientUser = await prisma.appUser.findUniqueOrThrow({ where: { email: 'system-portal-client@one17.test' } });
    await expectReject('adversarial (v): the client (represented by the SYSTEM_PORTAL_CLIENT initiator) cannot verify its own client-declared item', () => zakat.verifyScheduleItem(clientCashWf.id, systemPortalClientUser.id));
    const clientItemVerified = await workflowInbox.approve(clientCashWf.id, bd2.id);
    ...
    const { item: bdStaffItem, workflowInstance: bdStaffWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runC.id, scheduleType: 'GOLD_SILVER', description: 'BD-declared jewelry', amountKobo: 1000000n, zakatability: 'ZAKATABLE', declaredByUserId: bd1.id,
    });
    ...
    await expectReject('invariant 71(a)(i): certification is still refused -- the newly-declared GOLD_SILVER item is now the outstanding one', () => zakat.submitForCertification(runC.id, bd1.id));
    const { item: rejectedItem, workflowInstance: rejectedWf } = await zakat.declareScheduleItem({
        zakatAssessmentRunId: runC.id, scheduleType: 'RECEIVABLE', description: 'Disputed receivable, later rejected by a reviewer', amountKobo: 500000n, zakatability: 'ZAKATABLE', declaredByUserId: bd1.id,
    });
    await expectReject('invariant 71(a)(i): certification refused -- both GOLD_SILVER and the just-declared RECEIVABLE item are outstanding', () => zakat.submitForCertification(runC.id, bd1.id));
    ...
    await expectReject('invariant 71(a)(i): certification STILL refused -- the REJECTED item no longer blocks, but GOLD_SILVER remains outstanding', () => zakat.submitForCertification(runC.id, bd1.id));
    ...
    await expectReject('adversarial: a BD officer (holds ZAKAT_ADVISORY but NOT ZAKAT_PORTFOLIO_ADVISORY_FEED) is structurally blocked from the Portfolio feed -- the two capabilities are deliberately separate', () => zakat.getPortfolioAdvisoryFeed(investorC.investorId, bd1.id));
    ...
    if (advisoryFeedRound1.isActive === true &&
    ...
    const bdItemVerifiedByRelOff = await workflowInbox.approve(bdStaffWf.id, relOff.id);
    ...
    if (runCAfterSubmit.status === 'AWAITING_SHARIAH_CERTIFICATION' && runCAfterSubmit.netZakatableKobo === 51000000n) {
    ...
    const certifiedC = await workflowInbox.approve(certWfC.id, shariahReviewer.id);
    ...
    await expectReject('invariant 71(a)(i): runC, now CERTIFIED, refuses a further CLIENT-portal declaration too', () => zakat.declareScheduleItemAsClient(investorC.investorId, { zakatAssessmentRunId: runC.id, scheduleType: 'CASH_BANK', description: 'late client addition', amountKobo: 1n, zakatability: 'ZAKATABLE' }));
...
//# sourceMappingURL=zakat.smoke.js.map