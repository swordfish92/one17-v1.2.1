...
async function main() {
    ...
    const composer = new dashboard_composer_service_1.DashboardComposerService(prisma, audit, delegation);
    ...
    const makeUser = async (label, roleCodes, orgUnitCode, position) => {
    ...
    const COMPOSER_METRIC_FIXTURES = [
    ...
    for (const def of COMPOSER_METRIC_FIXTURES) {
        ...
        if (!existing)
            await prisma.metricDefinition.create({ data: { ...def, createdByUserId: systemUser.id } });
    }
    ...
    await expectReject('cannot compose a tile for a metric gated by a capability the actor does not hold', () => composer.saveDashboard({ name: 'Tampered', scope: 'PERSONAL', tiles: [{ metricCode: 'COMPOSER.OPEN_COMPLAINTS', presentation: 'KPI_TILE', position: 0 }] }, procOfficer.id));
    ...
    if (personalDash.scope === 'PERSONAL' && personalDash.tiles.length === 1)
        ok('procOfficer saves a PERSONAL dashboard with an eligible tile');
    else
        fail('PERSONAL dashboard did not save as expected', personalDash);
    await expectReject('a seat holding metric VIEW but not DASHBOARD_COMPOSER_PUBLISH cannot publish a DEPARTMENT dashboard', () => composer.saveDashboard({ name: 'Should Fail', scope: 'DEPARTMENT', tiles: [{ metricCode: 'COMPOSER.ISSUED_PURCHASE_ORDERS', presentation: 'KPI_TILE', position: 0 }] }, procOfficer.id));
    await expectReject('a HoD cannot publish a DEPARTMENT dashboard to a different org unit than their own', () => composer.saveDashboard({ name: 'Cross-Dept Attempt', scope: 'DEPARTMENT', orgUnitCode: orgUnitB, tiles: [{ metricCode: 'COMPOSER.ISSUED_PURCHASE_ORDERS', presentation: 'KPI_TILE', position: 0 }] }, deptChiefA.id));
    ...
    if (deptDash.scope === 'DEPARTMENT' && deptDash.orgUnitCode === orgUnitA)
        ok('DEPT_CHIEF publishes a DEPARTMENT dashboard, server-forced to their own org unit');
    else
        fail('DEPARTMENT dashboard did not save with the expected forced org unit', deptDash);
    const viewSameDept = await composer.getDashboard(deptDash.id, staffSameDeptA.id);
    const tileSameDept = viewSameDept.tiles[0];
    ...
    await expectReject('a different-department viewer cannot open a DEPARTMENT dashboard published to another org unit, even holding the same metric capability', () => composer.getDashboard(deptDash.id, staffOtherDeptB.id));
    const viewOutsider = await composer.getDashboard(deptDash.id, outsiderNoProc.id);
    const tileOutsider = viewOutsider.tiles[0];
    ...
    if (ownPersonalView.tiles[0].redacted === false)
        ok('the PERSONAL dashboard\'s own owner sees their own resolved tile');
    else
        fail('owner could not view their own PERSONAL dashboard tile resolved', ownPersonalView.tiles[0]);
    ...
    if (createdAudit.length === 2 && deniedAudit.length >= 1)
        ok('every dashboard creation and every save-time RBAC refusal is audit-logged (no approval workflow -- a view moves no money)');
    else
        fail('expected audit_trail rows for CREATE and PERMISSION_DENIED were not both found', { createdAudit: createdAudit.length, deniedAudit: deniedAudit.length });
...
//# sourceMappingURL=dashboard-composer.smoke.js.map