...
function scanRoutes() {
    ...
    for (const file of walk(srcDir)) {
        ...
        const capMatches = [];
        let capMatch;
        while ((capMatch = capRe.exec(text)))
            capMatches.push({ index: capMatch.index, code: capMatch[1] });
        const methodMatches = [];
        let m;
        while ((m = methodRe.exec(text)))
            methodMatches.push({ index: m.index, verb: m[1].toUpperCase(), subPath: m[2] ?? '' });
...
async function main() {
    ...
    const gitRev = (() => {
        try {
            return require('child_process').execSync('git rev-parse --short HEAD', { cwd: REPO_ROOT }).toString().trim();
        }
        catch {
            return 'uncommitted';
        }
...
//# sourceMappingURL=generate-controls-register.js.map