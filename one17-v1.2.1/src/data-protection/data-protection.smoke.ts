...
// Run with: npx tsx src/data-protection/data-protection.smoke.ts
// Invariant 57 (CHECK15, NDPA/GAID compliance pack) adversarial set across
// all 5 build items: (i) privacy notice + acknowledgment log, (ii) DSR
// tagged-Complaint response clock + legal-basis-required resolution, (iii)
// retention schedule registry confirm-once discipline, (iv) breach register
// full lifecycle + KRI CPL-06, (v) AI data-minimization record.
...
import { DataProtectionError } from './data-protection.types';
...
async function main() {
  ...
  const retentionSchedule = new RetentionScheduleService(prisma, audit);
  const breachRegister = new DataBreachRegisterService(prisma, audit);
  const kri = new KriEngineService(prisma, audit);
  ...
  // ==========================================================================
  // (i) Privacy notice + acknowledgment log
  // ==========================================================================
  ...
  if (log.some((a) => a.id === ack.id)) ok('recordAcknowledgment() writes a row findable via listAcknowledgments() (who/when/which-version)');
  else fail('acknowledgment not found in log', ack);
  await expectReject('acknowledging with no investor/counterparty/public-intake target is refused', () =>
    privacyNotice.recordAcknowledgment({ noticeVersion: activeNotice.version, context: 'PUBLIC_INTAKE' }));

  // ==========================================================================
  // (ii) DSR intake on the Enquiries channel: statutory clock + legal basis
  // ==========================================================================
  ...
  await expectReject('raising a DSR without a dsrCategory is refused', () =>
    complaints.raiseFromPortal({ investorId: investor.investorId, category: 'DSR', description: 'no category', isDsr: true }));

  const queue = await complaints.listDsrQueue(compliance.id);
  if (queue.some((q) => q.id === dsrComplaint.id)) ok('listDsrQueue() (the Compliance queue) finds the DSR-tagged complaint, with clientName + responseClockState');
  else fail('DSR queue did not find the tagged complaint', queue.map((q) => q.id));

  await expectReject('resolving a DSR complaint without a dsrLegalBasis is refused', () =>
    complaints.resolve({ complaintId: dsrComplaint.id, resolutionNotes: 'Erasure declined.', actorUserId: compliance.id }));
  ...
  // ==========================================================================
  // (iii) Retention schedule registry: confirm-once discipline
  // ==========================================================================
  ...
  if (draftRow) ok(`retention schedule ships ${beforeConfirm.filter((r) => r.status === 'DRAFT').length} DRAFT row(s) with retentionPeriodMonths=NULL — "never invent a value" pattern`);
  else fail('expected at least one DRAFT retention_schedule_entry with a NULL period', beforeConfirm);
  ...
  if (draftRow) {
    ...
    await expectReject('confirming an already-ACTIVE retention schedule entry is refused (version a new one instead)', () =>
      retentionSchedule.confirm({ entryId: draftRow.id, retentionPeriodMonths: 12, statutoryBasis: 'x', disposalTreatment: 'x', actorUserId: compliance.id }));
  }

  // ==========================================================================
  // (iv) Breach register: full lifecycle + KRI CPL-06
  // ==========================================================================
  ...
  if (afterLogCount === beforeOpenCount + 1) ok('KRI CPL-06 (getOpenCount) increments on a freshly logged, unclosed breach');
  else fail('open breach count did not increment as expected', { beforeOpenCount, afterLogCount });

  await expectReject('closing a breach before assessment is refused', () =>
    breachRegister.close({ breachId: breach.id, postIncidentNotes: 'x', actorUserId: compliance.id }));
  ...
  if (assessed.status === 'ASSESSED' && assessed.severity === 'MODERATE') ok('assess() records severity/eligibility and flips OPEN -> ASSESSED');
  else fail('assess() did not persist correctly', assessed);

  await expectReject('closing before NDPC notification is refused', () =>
    breachRegister.close({ breachId: breach.id, postIncidentNotes: 'x', actorUserId: compliance.id }));

  const notified = await breachRegister.recordNdpcNotification({ breachId: breach.id, actorUserId: compliance.id });
  if (notified.status === 'NDPC_NOTIFIED' && notified.ndpcNotifiedAt) ok('recordNdpcNotification() flips ASSESSED -> NDPC_NOTIFIED, timestamp recorded');
  else fail('recordNdpcNotification() did not persist correctly', notified);
  ...
  if (closed.status === 'CLOSED' && closed.closedByUserId === compliance.id) ok('close() succeeds once NDPC-notified and data-subject notification (not required) is satisfied');
  else fail('close() did not persist correctly', closed);

  const afterCloseCount = await breachRegister.getOpenCount();
  if (afterCloseCount === beforeOpenCount) ok('KRI CPL-06 decrements back to baseline once the breach is CLOSED');
  else fail('open breach count did not decrement after closure', { beforeOpenCount, afterCloseCount });
  ...
  // ==========================================================================
  // (v) AI data-minimization guard: auditable record of personal-data points
  // ==========================================================================
  ...
  const personalDataIncluded = aiLogRow.personalDataPointCodesIncluded as string[];
  ...
  if ((nonPiiLogRow.personalDataPointCodesIncluded as string[]).length === 0) {
  ...
  // Cleanup. ai_interaction_log is insert-only (DB-trigger-enforced,
  // invariant 33d) -- the two log rows this test created are PERMANENT, so
  // corpServices (their askingUserId) is deliberately left in place rather
  // than attempting a delete that would violate the FK, same residue-
  // tolerance discipline every other AI-touching smoke test in this
  ...
  if (draftRow) await prisma.retentionScheduleEntry.update({ where: { id: draftRow.id }, data: { status: 'DRAFT', retentionPeriodMonths: null, statutoryBasis: null, disposalTreatment: null, confirmedByUserId: null, confirmedAt: null } }).catch(() => {});
