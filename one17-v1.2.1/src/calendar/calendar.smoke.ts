// Run with: npx tsx src/calendar/calendar.smoke.ts
// Invariant 55(c2): the Staff Calendar. Proves against the real live DB:
// personal-entry CRUD (owner-only), the cross-user privacy boundary for
// PRIVATE entries, meeting invite/accept/decline plotting on the
// attendee's own calendar, auto-plotted entries syncing in from Task and
// staying delete-proof and idempotent across repeat syncs, the ICS feed
// token lifecycle (issue/reuse/revoke), the reminder sweep's fire-once
// dedup guard, and busy-block free/busy lookups (accepted counts busy,
// declined does not).
...
async function main() {
  ...
  // --- Personal entries: owner-only CRUD, PRIVATE by construction --------
  ...
  // --- Meetings: invite -> plots on attendee's calendar -> accept/decline
  ...
  // --- Auto-plotted sync: Task deadlines, read-only, idempotent ----------
  ...
  // --- ICS feed token: issue, reuse, revoke -------------------------------
  ...
  // --- Reminder sweep: fires once, dedup guard holds on re-run -----------
  ...
  // --- Busy blocks: accepted counts busy, declined does not --------------
