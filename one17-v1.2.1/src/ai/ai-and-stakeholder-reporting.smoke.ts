// Run with: npx tsx src/ai/ai-and-stakeholder-reporting.smoke.ts
// Check-6B item 2: AI Services Layer foundation (invariant 33, amended (e)
// + new (f)) and Stakeholder Reporting distribution rules (invariant 24),
// proven live against the real DB. Six CEO-named adversarial proofs, in
// order: (1) flag-off inert even for SUPER_ADMIN; (2) granted-but-wrong-
// context refused; (3) the two-role scoping test; (4) token-budget
// exhaustion refusing gracefully; (5) kill switch dropping requests mid-
// flight; (6) stakeholder-reporting distribution rules (unapproved cannot
// distribute; CLIENT audience requires MD/DoA sign-off). Plus: both new
// immutable logs (ai_interaction_log, stakeholder_distribution_log)
// genuinely reject UPDATE at the DB level.
...
async function main() {
  ...
  // seed.ts's flag upsert deliberately preserves real toggled state across
  // reseeds (a live CEO-approved flip must survive a later `db seed` run —
  // same "never overwrite a live decision" reasoning as investor_onboarding_
  // config). That means this smoke test cannot assume the seed-time OFF
  // default; it must establish its own known baseline so it is safely
  // re-runnable regardless of what a prior partial run left behind.
  ...
  // ==========================================================================
  // Proof 1: flag-off inert even for SUPER_ADMIN.
  // ==========================================================================
  const chatFlagBefore = await prisma.aiCapabilityFlag.findUniqueOrThrow({ where: { capabilityCode: 'AI_CHAT' } });
  if (chatFlagBefore.isEnabled) fail('expected AI_CHAT to ship OFF by default (invariant 33e)', chatFlagBefore);
  ...
  // ==========================================================================
  // Flip AI_CHAT and AI_SHARIAH_ASSIST ON via the REAL DoA-governed
  // workflow (ICT initiates, MD_CEO approves) — proves the gate-1
  // management mechanism itself, not just a direct DB toggle.
  // ==========================================================================
  for (const capabilityCode of ['AI_CHAT', 'AI_SHARIAH_ASSIST']) {
    ...
    if (approved?.isEnabled === true) ok(`AI_CAPABILITY_FLAG_TOGGLE: MD_CEO approval flips ${capabilityCode} ACTIVE via the real DoA workflow (ICT initiates, MD_CEO approves)`);
    else fail(`${capabilityCode} flag toggle did not flip correctly`, approved);
  }

  // ==========================================================================
  // Proof 2: granted-but-wrong-context refused.
  // ==========================================================================
  ...
  // ==========================================================================
  // Proof 3: the two-role scoping test — identical question, differently-
  // scoped answers. CORP_SERVICES holds PROCUREMENT_OPERATIONS (sees
  // VENDOR_COUNT, not SCHEDULED_JOB_COUNT); ICT holds SCHEDULER_OPERATIONS
  // (the reverse) — both hold AI_CHAT.
  // ==========================================================================
  ...
  if (
    corpServicesAnswer.includedDataPointCodes.includes('VENDOR_COUNT') && !corpServicesAnswer.includedDataPointCodes.includes('SCHEDULED_JOB_COUNT') &&
  ...
  // ==========================================================================
  // Proof 4: token-budget exhaustion refusing gracefully.
  // ==========================================================================
  ...
  // ==========================================================================
  // Proof 5: kill switch dropping requests mid-flight.
  // ==========================================================================
  ...
  // ==========================================================================
  // ai_interaction_log immutability (invariant 33d).
  // ==========================================================================
  await expectReject('UPDATE on ai_interaction_log is rejected at the DB level (insert-only trigger)', () =>
    prisma.aiInteractionLog.update({ where: { id: refusalLog.id }, data: { responseText: 'tampered' } }));

  // ==========================================================================
  // Defect found and fixed (Check-6C tranche 3): One17AIService.
  // listInteractionLog() previously took no actor and applied NO capability
  // check at all, despite the controller's own comment claiming it was
  // gated on AI_AUDITLOG_QUERY — any authenticated user could read every
  // user's AI prompts/responses. Fixed to require the real check.
  // ==========================================================================
  ...
  // ==========================================================================
  // Proof 6: stakeholder-reporting distribution rules (invariant 24).
  // ==========================================================================
  ...
  await expectReject('distributing an unapproved (DRAFT) stakeholder report', () =>
    stakeholderReporting.distribute({ stakeholderReportRunId: internalReport.id, audienceClass: 'INTERNAL', recipientDescription: 'FinCon department circulation', distributedByUserId: finAdmin.id }));
  ...
  await expectReject('CLIENT audience distribution with NO sign-off supplied', () =>
    stakeholderReporting.distribute({ stakeholderReportRunId: clientReport.id, audienceClass: 'CLIENT', recipientDescription: 'Client portal delivery', distributedByUserId: finAdmin.id }));
  await expectReject('CLIENT audience distribution sign-off by a user who lacks STAKEHOLDER_REPORT_DISTRIBUTION_SIGNOFF', () =>
    stakeholderReporting.distribute({ stakeholderReportRunId: clientReport.id, audienceClass: 'CLIENT', recipientDescription: 'Client portal delivery', distributedByUserId: finAdmin.id, signOffByUserId: finAdmin.id }));
  ...
  await expectReject('UPDATE on stakeholder_distribution_log is rejected at the DB level (insert-only trigger)', () =>
    prisma.stakeholderDistributionLog.update({ where: { id: clientDistribution.id }, data: { recipientDescription: 'tampered' } }));

  // ==========================================================================
  // Invariant 37(c)(iv): Board Papers — reuses this SAME module with the
  // BOARD audience class; CS generates/distributes; the SAME sign-off-
  // required rule (invariant 24) now also covers BOARD.
  // ==========================================================================
  ...
  await expectReject('BOARD audience distribution with NO sign-off supplied (invariant 37c-iv extends invariant 24\'s DoA rule)', () =>
    stakeholderReporting.distribute({ stakeholderReportRunId: boardReport.id, audienceClass: 'BOARD', recipientDescription: 'Board pack circulation', distributedByUserId: cs.id }));
