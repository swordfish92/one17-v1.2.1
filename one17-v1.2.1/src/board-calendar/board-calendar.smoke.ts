// Run with: npx tsx src/board-calendar/board-calendar.smoke.ts
// Invariant 37(c)(iii): Board Calendar (meetings, committee cycles) with
// governed reminder notifications.
...
async function main() {
  ...
  await expectReject('an outsider cannot create a Board Calendar event', () =>
    boardCalendar.createEvent({ title: 'x', startsAt: new Date() }, outsiderUser.id));
  ...
  if (updatedConfig.daysBefore === 2) ok('CS updates the governed reminder lead time (invariant 10: no hardcoded threshold)');
  else fail('reminder config did not update', updatedConfig);

  const dueSoon = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000); // 1 day out — within the 2-day window
  const farOut = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days out — outside the window
  ...
  await expectReject('an outsider cannot submit a report pack', () =>
    boardCalendar.submitReportPack({ calendarEventId: eventDueSoon.id, title: 'x', fileReference: 'x' }, outsiderUser.id));

  const submission = await boardCalendar.submitReportPack(
    { calendarEventId: eventDueSoon.id, title: `Risk Committee pack ${RUN}`, fileReference: `/documents/pack-${RUN}.pdf` },
    mdUser.id,
  );
  ...
  await expectReject('an outsider cannot mark a submission received', () =>
    boardCalendar.markSubmissionReceived(submission.id, 'RECEIVED', undefined, outsiderUser.id));
  ...
  await expectReject('a submission already tracked (RECEIVED) cannot be marked again', () =>
    boardCalendar.markSubmissionReceived(submission.id, 'RECEIVED', undefined, csUser.id));

  const incompleteSubmission = await boardCalendar.submitReportPack(
    { calendarEventId: eventDueSoon.id, title: `Incomplete pack ${RUN}`, fileReference: `/documents/pack-incomplete-${RUN}.pdf` },
    mdUser.id,
  );
  ...
  await expectReject('an outsider cannot list submissions for an event', () =>
    boardCalendar.listSubmissionsForEvent(eventDueSoon.id, outsiderUser.id));
