// Run with: npx tsx src/document-transmission/document-transmission.smoke.ts
// Invariant 55(b) (CHECK12, CEO directive 8-Jul-2026): MD Document
// Transmission — MD transmits a document to named officers, each recipient
// gets a document-register row + notification + a REAL Task (when they have
// an Employee record), and acknowledgment is a distinct second action by
...
async function main() {
  ...
  const documentTransmission = new DocumentTransmissionService(prisma, audit, delegation, notification, task);
  ...
  // A recipient WITH an Employee record — gets the real Task.
  ...
  // A recipient WITHOUT an Employee record — external/role-only account,
  // still gets the register row + notification, just no Task.
  ...
  await expectReject('an outsider without MD_DOCUMENT_TRANSMISSION cannot transmit a document', () =>
    documentTransmission.transmitDocument({ title: 'x', fileReference: 'x', recipientUserIds: [recipientUser.id] }, outsiderUser.id));
  await expectReject('a CS user (VIEW only) cannot transmit a document — MD_CEO is the initiator', () =>
    documentTransmission.transmitDocument({ title: 'x', fileReference: 'x', recipientUserIds: [recipientUser.id] }, csUser.id));
  await expectReject('transmitting with zero recipients is rejected', () =>
    documentTransmission.transmitDocument({ title: 'x', fileReference: 'x', recipientUserIds: [] }, mdUser.id));

  const document = await documentTransmission.transmitDocument(
    { title: `Board resolution pack ${RUN}`, fileReference: `/documents/res-${RUN}.pdf`, docType: 'BOARD_RESOLUTION', recipientUserIds: [recipientUser.id, noEmployeeRecipientUser.id] },
    mdUser.id,
  );
  ...
  await expectReject('an outsider (not a recipient, no VIEW capability) cannot acknowledge a transmission never sent to them', () =>
    documentTransmission.acknowledgeTransmission(document.id, outsiderUser.id));
  ...
  await expectReject('a transmission already acknowledged cannot be acknowledged twice', () =>
    documentTransmission.acknowledgeTransmission(document.id, recipientUser.id));

  const mdView = await documentTransmission.listDocumentsForViewer(mdUser.id);
  const mdDoc = mdView.find((d: any) => d.id === document.id);
  ...
  if (recipientView.some((d: any) => d.id === document.id)) {
  ...
  if (!outsiderView.some((d: any) => d.id === document.id)) {
