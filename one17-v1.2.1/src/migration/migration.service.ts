...
// Phase 3: One17 Migration Data Standards v0.1 — TPL_01-07 (+ TPL_08-10 per
// PMS SDS §6, "loaders reuse the TPL staging + validation-gate machinery").
// Every template lands in the same generic staging shape
// (MigrationBatch/MigrationStagingRow); this service is the "six
// Reconciliation Gates" plus each template's row-level rules, applied as
// CODE against DATA rather than ten bespoke pipelines. Promotion is
// idempotent throughout (upsert on migrationSourceRef) — same discipline as
// seed.ts's upsertApprovalRule (closes-D8 pattern) — so a batch can be
// re-validated and re-promoted any number of times with no duplicate rows.
@Injectable()
export class MigrationService {
  ...
  async validateBatch(batchId: string, actorUserId: string) {
    ...
    const validator = this.gateFnFor(batch.tplCode as TplCode);
    ...
    for (const row of rows) {
      ...
      await this.prisma.migrationStagingRow.update({
        where: { id: row.id },
        data: { status, rejectionReasons: failures as any },
      });
    }
  ...
  async promoteBatch(batchId: string, actorUserId: string) {
    await this.assertCapability(actorUserId, 'DATA_MIGRATION', 'INITIATE', 'promote a migration batch');

    const batch = await this.prisma.migrationBatch.findUniqueOrThrow({ where: { id: batchId } });
    if (batch.status !== 'VALIDATED') {
      throw new MigrationValidationError(`Batch ${batchId} must be VALIDATED before promotion (status is ${batch.status}).`);
    }
    const rows = await this.prisma.migrationStagingRow.findMany({
      where: { batchId, status: 'VALID' },
      orderBy: { rowNumber: 'asc' },
    });
    const promoter = this.promoterFor(batch.tplCode as TplCode);
  ...
  // (task 34, Migration Data Standards v0.1) existed as a full service since
  // Phase 2 but had only ever been driven by one-off scripts (Check-2/3) —
  // no live screen. VIEW-only, no capability check of its own (matches
  // DocumentController's listByEntity precedent).
  async listBatches() {
  ...
  async generateValidationReport(batchId: string): Promise<string> {
    const batch = await this.prisma.migrationBatch.findUniqueOrThrow({ where: { id: batchId } });
    const rows = await this.prisma.migrationStagingRow.findMany({ where: { batchId }, orderBy: { rowNumber: 'asc' } });
    ...
    if (rejected.length === 0) {
      ...
      for (const row of rejected) {
        ...
        for (const f of row.rejectionReasons as unknown as GateFailure[]) {
  ...
  // ==========================================================================
  // Reconciliation Gates (Migration Data Standards §"Reconciliation Gates" —
  // run across ALL promoted data, not per-batch; "migration is not done
  // until all pass").
  // ==========================================================================
  async runReconciliationGates(): Promise<ReconciliationGateResult[]> {
    const results: ReconciliationGateResult[] = [];

    // Gate 1: per product account, sum of TPL_05 txns = current balance per
    // latest investor statement. No independent "latest statement" source
    // exists outside the migration itself, so this is implemented as
    // internal consistency: every promoted ProductAccount's principal +
    // signed txn history nets to a coherent non-negative position for fund
    // accounts (units) — see detail. Documented as a scoped interpretation,
    // not the full external reconciliation the README envisions (that needs
    // a live Navision feed this migration doesn't have).
    const accounts = await this.prisma.productAccount.findMany({ include: { txns: true } });
    let gate1Bad: string[] = [];
    for (const acc of accounts) {
      if (acc.unitsHeld == null) continue;
    ...
    // Gate 2: Halal Fund total units across TPL_04 = fund's units in issue.
    ...
    // Gate 3: Mudarabah pool: sum of capital balances = pool capital per
    // last profit period.
    ...
    // Gate 4: TPL_07 debits = credits within each ledger entity separately.
    ...
    let gate4Bad: string[] = [];
    const byEntity = new Map<string, { dr: bigint; cr: bigint }>();
    ...
    for (const [entity, { dr, cr }] of byEntity) {
      if (dr !== cr) gate4Bad.push(`${entity}: debits=${dr} credits=${cr}`);
    }
    results.push({
      gate: 4,
    ...
    // Gate 5: every TPL_04 account maps to an investor in TPL_01; every
    // TPL_05 row maps to an account in TPL_04. Both are mandatory, enforced
    // FK relations (product_account.investor_id, txn.product_account_id
    // for migrated rows) — an orphan cannot be INSERTed at all, so this
    // gate is satisfied by construction rather than by a post-hoc scan;
    // reported here as the row counts that prove the relations are wired.
    ...
    // Gate 6: overlap check — count of distinct BVN/RC in TPL_01 = count of
    // rows (no duplicates survived).
    const investors = await this.prisma.investor.findMany({
      where: { migrationSourceRef: { not: null } },
      select: { bvn: true, rcNumber: true },
    });
    const keys = investors.map((i) => i.bvn ?? i.rcNumber).filter((k): k is string => !!k);
  ...
  // ==========================================================================
  // Row-level gate dispatch
  // ==========================================================================
  private gateFnFor(tplCode: TplCode): (raw: Record<string, string>, allRows: { rawData: unknown }[]) => Promise<GateFailure[]> | GateFailure[] {
    const map: Record<TplCode, (raw: Record<string, string>, allRows: { rawData: unknown }[]) => Promise<GateFailure[]> | GateFailure[]> = {
  ...
  private validateTpl01(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
  ...
  private validateTpl02(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
  ...
  private validateTpl03(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    requireField(raw, 'temp_investor_ref', f);
  ...
  private validateTpl04(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    ...
    // TPL_04 v2 (CLAUDE.md 21a-iii): AMD-01 — pool PSR is product-level
    // (ProductParameters), never per-account. Loader REJECTS per-account PSR
    // on POOL products (prior demo rows violated this and passed; the fix).
  ...
  private validateTpl05(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    ...
    // Rule 1: fund txns must carry units + unit_price; amount ~= units *
    // unit_price within +-0.5%.
    if (isFund) {
    ...
    // Rule 2: pool txns must NOT carry units.
    if (isPool && hasUnits) {
      f.push({ gate: 'rule-2-pool-no-units', reason: 'pool transaction carries a units value; pool transactions must not carry units' });
    }
    // Rule 3: PROFIT_ALLOCATION must reference an actual profit period, not
    // a projected/target figure.
    ...
    // Rule 4: source_system + source_document_ref required.
  ...
  private validateTpl06(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    ...
    // TPL_06 v2 (CLAUDE.md 21a-iv): control C13 — "no new deployment on
    // expired certs." A counterparty whose cert is ALREADY expired at
    // migration load time is flagged for FinCon/SAU review rather than
    // silently promoted (same "flagged = hard stop for human review"
    // discipline as TPL_01's no-BVN/NIN gate) — the counterparty still
    // enters the register once cleared; only forward deployment is gated.
  ...
  private validateTpl07(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
  ...
  // SDS §6: "TPL_08 reports_to must resolve; position/cadre must exist in
  // TPL_09." The cadre check queries already-PROMOTED emolument_structure
  // rows — this makes TPL_09's promotion a precondition for a clean TPL_08
  // validation run, per the load order this smoke test follows (TPL_09
  // before TPL_08). Cadres with no matching emolument_structure entry are a
  // real, correctly-reported rejection, not a bug — see the CHECK3
  // deviations register for the shipped TPL_08/TPL_09 demo data's own gap
  // (CEO and Analyst 1 appear in TPL_08 but TPL_09's demo rows only cover
  // Snr Associate 1).
  private async validateTpl08(raw: Record<string, string>, allRows: { rawData: unknown }[]): Promise<GateFailure[]> {
    const f: GateFailure[] = [];
    ...
    if (raw.reports_to_ref) {
      const resolves = allRows.some((r) => (r.rawData as Record<string, string>).temp_employee_ref === raw.reports_to_ref);
      if (!resolves) {
  ...
  // SDS §6: "TPL_09 Sigma components = total emolument +-N1, kobo integers."
  // No external "total emolument" figure exists inside TPL_09 itself — see
  // MigrationService's class comment / CHECK3 deviations register: the
  // total is DEFINED as the sum of a (cadre, notch) group's components, so
  // the only independently checkable invariant here is "kobo integers"
  // (<=2 decimal places), which this enforces.
  private validateTpl09(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    requireField(raw, 'cadre', f);
  ...
  // SDS §6: "TPL_10 Sigma weights per dept x tier = 100%, class in {...},
  // tier in {...}." weight_pct is demo-blank in the shipped template ("weight
  // from dept x tier matrix" — real weights live in kpi_weight_matrix), so
  // the sum check only applies when populated; documented as a deviation.
  private validateTpl10(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
    requireField(raw, 'org_unit', f);
  ...
  // TPL_13 (CLAUDE.md 21a-ii): one flat CSV, four record_types sharing
  // generic columns reinterpreted per type — same convention as every other
  // TPL. record_ref is the natural key regardless of type (member ref /
  // resolution ref / purification ref / check code).
  private validateTpl13(raw: Record<string, string>): GateFailure[] {
    const f: GateFailure[] = [];
  ...
  // ==========================================================================
  // Promotion dispatch (idempotent: upsert on migrationSourceRef)
  // ==========================================================================
  private promoterFor(tplCode: TplCode): (raw: Record<string, string>, actorUserId: string) => Promise<string> {
    const map: Record<TplCode, (raw: Record<string, string>, actorUserId: string) => Promise<string>> = {
  ...
  private async promoteTpl01(raw: Record<string, string>, actorUserId: string): Promise<string> {
    ...
    const investor = await this.prisma.investor.upsert({
      ...
      create: {
        ...
        amlRiskRating: raw.aml_risk_rating as any,
        bvn: raw.bvn || null,
        nin: raw.nin || null,
  ...
  private async promoteTpl02(raw: Record<string, string>): Promise<string> {
  ...
  private async promoteTpl03(raw: Record<string, string>, actorUserId: string): Promise<string> {
    const investor = await this.prisma.investor.findFirstOrThrow({ where: { migrationSourceRef: raw.temp_investor_ref } });
    ...
    const created = await this.prisma.investorKycDocument.create({
      data: {
        ...
        verificationStatus: (raw.verification_status as any) || 'VERIFIED',
  ...
  private async promoteTpl04(raw: Record<string, string>): Promise<string> {
    const investor = await this.prisma.investor.findFirstOrThrow({ where: { migrationSourceRef: raw.temp_investor_ref } });
    const account = await this.prisma.productAccount.upsert({
      ...
      // Re-promotion re-syncs the driver/PSR/election fields to the current
      // validated source row rather than leaving whatever was captured on
      // first promotion — otherwise a TPL_04 v2 correction (e.g. removing an
      // erroneous per-account PSR value on a POOL product) would pass
      // validation on re-run but never actually heal the already-promoted
      // row, defeating the point of the AMD-01 gate.
      update: {
        psrInvestorPct: raw.psr_investor_pct ? Number(raw.psr_investor_pct) : null,
        psrMudaribPct: raw.psr_mudarib_pct ? Number(raw.psr_mudarib_pct) : null,
        dripElection: raw.drip_election || null,
  ...
  private async promoteTpl05(raw: Record<string, string>, actorUserId: string): Promise<string> {
  ...
  private async promoteTpl06(raw: Record<string, string>): Promise<string> {
  ...
  private async promoteTpl07(raw: Record<string, string>, actorUserId: string): Promise<string> {
    ...
    if (!account) {
      // A migration-sourced account not in the standard template — created
      // with placeholder framework refs, flagged for FINCON's mapping
      // review (AMD-11.1 requires both, this is a documented gap, not a
      // guess at the real classification).
    ...
    if (existingLine) return existingLine.id;
  ...
  private async promoteTpl08(raw: Record<string, string>): Promise<string> {
    ...
    const employee = await this.prisma.employee.upsert({
      ...
      create: {
        ...
        status: (raw.employment_status as any) || 'ACTIVE',
    ...
    // TPL_08 v2 carries the invariant-29(a) PERSONAL RECORDS fields
    // natively — promote them into employee_personal_record alongside the
    // employee row itself, same upsert-on-migration discipline.
    if (raw.marital_status || raw.number_of_children || raw.residential_address || raw.next_of_kin_name || raw.hobbies_interests) {
      await this.prisma.employeePersonalRecord.upsert({
        ...
        create: {
          ...
          maritalStatus: (raw.marital_status as any) || null,
  ...
  private async promoteTpl09(raw: Record<string, string>): Promise<string> {
    ...
    const created = await this.prisma.emolumentStructure.create({
      data: {
        ...
        componentType: raw.component_type as any,
  ...
  private async promoteTpl10(raw: Record<string, string>): Promise<string> {
    const orgUnit = await this.prisma.orgUnit.upsert({
      where: { code: raw.org_unit },
      create: { code: raw.org_unit, name: raw.org_unit },
      update: {},
    ...
    const kpi = await this.prisma.kpiDefinition.upsert({
      where: { kraCode_tier: { kraCode: kra.code, tier: raw.tier as any } },
      create: {
        kraCode: kra.code,
        tier: raw.tier as any,
        kpiText: raw.kpi_text,
        kpiClass: raw.kpi_class.toUpperCase() as any,
        objectiveText: raw.strategic_objective || null,
        exampleActivity: raw.example_activity || null,
        measurementBasis: (raw.measurement_basis as any) || 'MANUAL',
  ...
  private async promoteTpl13(raw: Record<string, string>): Promise<string> {
    switch (raw.record_type) {
      ...
      case 'SSB_RESOLUTION': {
        let proposedByMemberId: string | null = null;
      ...
      default: {
        let reviewedByMemberId: string | null = null;
  ...
  private async nextInvestorId(): Promise<string> {
    const rows = await this.prisma.$queryRaw<{ nextval: bigint }[]>`SELECT nextval('investor_id_seq')`;
  ...
  private async assertCapability(userId: string, functionCode: string, level: 'INITIATE' | 'APPROVE' | 'VIEW', activity: string) {
    ...
    if (!eligible) {
      ...
      throw new MigrationValidationError(`User lacks ${level} authority on ${functionCode} (standing or delegated), required to ${activity}.`);
    }
  }
}

function requireField(raw: Record<string, string>, field: string, f: GateFailure[], gate = 'structural') {
  if (!raw[field] || raw[field].trim() === '') {
    f.push({ gate, reason: `required field '${field}' is missing or blank` });
  }
}

function checkDate(raw: Record<string, string>, field: string, f: GateFailure[]) {
  const v = raw[field];
  if (!v) return;
...
function checkAmount(raw: Record<string, string>, field: string, f: GateFailure[], opts: { allowNegative: boolean; gate?: string }) {
  const v = raw[field];
...
function nairaToKobo(naira: string): bigint {
