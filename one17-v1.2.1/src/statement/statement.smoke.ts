...
// Run with: npx tsx src/statement/statement.smoke.ts
// Invariant 44(d) (CHECK10): portal investor statements. Own SMOKE-prefixed
// fixtures throughout -- never touches the walkthrough DB or its real data.
...
import { StatementError } from './statement.types';
...
// pdf-lib writes drawText() strings as hex-encoded PDF string literals
// (<48656C6C6F>) inside FlateDecode-compressed content streams -- neither
// a plain buffer.toString() nor a naive substring search sees the actual
// words. Inflate every stream object and hex-encode the search needle to
// match pdf-lib's own encoding, so this sweep tests what the PDF ACTUALLY
// renders rather than a string that merely appears somewhere in the file.
function extractPdfHexText(bytes: Uint8Array): string {
  ...
  while ((match = streamRe.exec(raw))) {
    ...
    try { chunks.push(zlib.inflateSync(streamBytes).toString('latin1')); } catch { /* not a flate stream (e.g. binary xref) -- skip */ }
  }
  return chunks.join('\n');
}
...
async function main() {
  ...
  // ---- Fixture: UNIT_NAV (Halal Fund) holding for investor A ----
  ...
  const fundAccount = await prisma.productAccount.create({
    data: { investorId: investorA.investorId, productCode: FUND_CODE, startDate: new Date('2026-01-01'), principalOrCommittedKobo: 5_000_000_00n, unitsHeld: 50000 },
  });
  await prisma.navRecord.create({
    data: {
      ledgerEntityCode: fundLedger.code, valuationDate: new Date('2026-03-31'), portfolioMktValKobo: 550_000_000_00n,
      uninvestedCashKobo: 10_000_00n, accruedUnpaidFeesKobo: 500_00n, totalNavKobo: 550_010_000_00n,
      unitsOutstanding: 5_000_000, navPerUnit: 110.002, offerPrice: 110.5, bidPrice: 109.5,
    },
  });
  await prisma.txn.create({
    data: { txnType: 'SUBSCRIPTION', ledgerEntityCode: fundLedger.code, productAccountId: fundAccount.id, valueDate: new Date('2026-01-05'), amountKobo: 5_000_000_00n, unitsQty: 50000, makerUserId: staff.id, status: 'POSTED' },
  });
  const fundDistribution = await prisma.distribution.create({
    data: { ledgerEntityCode: fundLedger.code, productCode: FUND_CODE, method: 'NAV', periodStart: new Date('2026-01-01'), periodEnd: new Date('2026-03-31'), recordDate: new Date('2026-03-31'), netAvailableKobo: 1_000_000_00n, toParticipantsKobo: 900_000_00n, retainedOrMudaribBaseKobo: 100_000_00n, paidAt: new Date('2026-04-05'), createdByUserId: staff.id },
  });
  await prisma.distributionLineItem.create({
    data: { distributionId: fundDistribution.id, productAccountId: fundAccount.id, capitalKobo: 5_000_000_00n, totalPayoutKobo: 45_000_00n, dripUnits: 400 },
  });

  // ---- Fixture: PSR_WATERFALL (Pool) holding for investor A ----
  ...
  const poolAccount = await prisma.productAccount.create({
    data: { investorId: investorA.investorId, productCode: POOL_CODE, startDate: new Date('2026-01-01'), principalOrCommittedKobo: 20_000_000_00n, psrInvestorPct: 0.7, psrMudaribPct: 0.3, targetReturnPctBenchmark: 0.15 },
  });
  const poolDistribution = await prisma.distribution.create({
    data: { ledgerEntityCode: poolLedger.code, productCode: POOL_CODE, method: 'ND_MUDARABAH', periodStart: new Date('2026-01-01'), periodEnd: new Date('2026-03-31'), recordDate: new Date('2026-03-31'), netAvailableKobo: 3_000_000_00n, toParticipantsKobo: 2_100_000_00n, retainedOrMudaribBaseKobo: 900_000_00n, paidAt: new Date('2026-04-05'), createdByUserId: staff.id },
  });
  await prisma.distributionLineItem.create({
    data: { distributionId: poolDistribution.id, productAccountId: poolAccount.id, capitalKobo: 20_000_000_00n, priorityPayoutKobo: 250_000_00n, surplusPayoutKobo: 30_000_00n, totalPayoutKobo: 280_000_00n },
  });

  // ---- Fixture: MANDATE (ND-PSR) participation for investor A ----
  ...
  const mandateAccount = await prisma.ndMandateAccount.create({
    data: { mandateRef: MANDATE_REF, ledgerEntityCode: mandateLedger.code, participantType: 'INVESTOR', investorId: investorA.investorId, sharingMode: 'MUDARABAH_PSR', investorRatio: 0.75, mudaribRatio: 0.25, targetReturnPct: 0.14, status: 'ACTIVE', startDate: new Date('2026-01-01'), committedCapitalKobo: 10_000_000_00n, createdByUserId: staff.id },
  });
  const mandateDistribution = await prisma.distribution.create({
    data: { ledgerEntityCode: mandateLedger.code, productCode: MANDATE_REF, method: 'ND_MUDARABAH', periodStart: new Date('2026-01-01'), periodEnd: new Date('2026-03-31'), recordDate: new Date('2026-03-31'), netAvailableKobo: 1_500_000_00n, toParticipantsKobo: 1_125_000_00n, retainedOrMudaribBaseKobo: 375_000_00n, paidAt: new Date('2026-04-05'), createdByUserId: staff.id },
  });
  await prisma.distributionLineItem.create({
    data: { distributionId: mandateDistribution.id, ndMandateAccountId: mandateAccount.id, capitalKobo: 10_000_000_00n, priorityPayoutKobo: 100_000_00n, surplusPayoutKobo: 12_500_00n, totalPayoutKobo: 112_500_00n },
  });

  // ---- 1. listAccounts returns all three holdings, correctly typed ----
  ...
  // ---- 2. UNIT_NAV statement generates a real PDF, contains units/NAV vocabulary ----
  ...
  // ---- 3. PSR (pool) statement -- structurally CANNOT carry units/NAV vocabulary ----
  ...
  // ---- 4. MANDATE (ND-PSR) statement -- same structural guarantee ----
  ...
  // ---- 5. Cross-client leak: investor B cannot fetch investor A's statements ----
  await expectReject("investor B cannot fetch investor A's UNIT_NAV statement by guessing the account id", () =>
    statements.generateStatementPdf('PRODUCT', fundAccount.id, investorB.investorId, periodStart, periodEnd));
  await expectReject("investor B cannot fetch investor A's POOL statement by guessing the account id", () =>
    statements.generateStatementPdf('PRODUCT', poolAccount.id, investorB.investorId, periodStart, periodEnd));
  await expectReject("investor B cannot fetch investor A's MANDATE statement by guessing the account id", () =>
    statements.generateStatementPdf('MANDATE', mandateAccount.id, investorB.investorId, periodStart, periodEnd));

  // ---- 6. Unknown id names the gap rather than throwing an opaque error ----
  await expectReject('unknown product account id is refused with a clear message', () =>
    statements.generateStatementPdf('PRODUCT', '00000000-0000-0000-0000-000000000000', investorA.investorId, periodStart, periodEnd));

  if (!(new StatementError('x') instanceof Error)) fail('StatementError should extend Error');
