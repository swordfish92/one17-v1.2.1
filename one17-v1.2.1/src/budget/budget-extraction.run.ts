// Task 47 runner (Budget Spec §5a, TPL_11 v2 driver model), not a pass/fail
// smoke test — this is the actual extraction, producing the per-sheet report
// for FinCon sign-off. Run with: npx tsx src/budget/budget-extraction.run.ts
...
import { ClassifiedBudgetLine, GateResult } from './budget.types';
...
async function main() {
  ...
  const svc = new BudgetExtractionService(prisma, audit, delegation);
  ...
  // Three scenario versions: only Scenario 2 has exported source data
  // (confirmed by BQ-6's own "staff cost monthly x12 = scenario-2 total"
  // cross-check) — 1 and 3 load as empty DRAFT shells, honestly documented.
  ...
  // --- Sheet 05: control totals (validation reference only, never loaded as budget_line) ---
  ...
  for (const line of sheet05) controlTotals.set(normalizeControlLabel(line.label), line.monthlyValues);
  ...
  // --- Sheet 02: income drivers (rows 23 mobilization, 25 FUM, 38/40 income) ---
  ...
  for (const [tag, line] of [['MOBILIZATION_MONTHLY', mobilizationLine], ['FUM_MONTHLY', fumLine], ['INCOME_TI_MONTHLY', incomeTiLine], ['NPT_INCOME_MONTHLY', nptIncomeLine]] as const) {
    if (!line) { reportLines.push(`- ${tag}: NOT FOUND in sheet 02 — check row labels`); continue; }
    for (let m = 0; m < 12; m++) {
  ...
  reportLines.push('');
  if (!mobilizationLine || !fumLine) throw new Error('Cannot proceed without mobilization/FUM driver series from sheet 02.');
  const driverSeries = { FUM: fumLine.monthlyValues, MOBILIZATION: mobilizationLine.monthlyValues };

  // --- Sheet 04: Other Income -> single budget_target (documented simplification, sheet 04 itself is scattered blocks) ---
  const otherIncomeControl = controlTotals.get('OTHER INCOME');
  if (otherIncomeControl) {
    for (let m = 0; m < 12; m++) await svc.saveBudgetTarget(v2.id, 'OTHER_INCOME_MONTHLY', m + 1, otherIncomeControl[m], 'KOBO', '04 (via sheet-05 control total — sheet 04 itself is scattered rent/Sukuk blocks per BQ-4/5, not a clean monthly line series)');
  ...
  // --- Sheet 07: opex detail (to row 138) ---
  ...
  // endRow=10: row 10 (0-indexed 9) is the bare "Total" line; everything
  // from row 13 on is a differently-shaped "Details" sub-ledger (S/N, Date,
  // Details, Amount, Ranking% columns) that must never be read as Jan-Dec
  // monthly cells — bounding the scan here, not a generic "keep reading
  // until blank" pass, is what keeps the two tables from colliding.
  ...
  // --- Gate A: monthly sum = FY total per line ---
  ...
  if (gateAFailed.length) reportLines.push(gateAFailed.map((g) => `- FAIL: ${g.detail}`).join('\n'));
  reportLines.push('');

  // --- Gate B: detail sum per group per month = sheet-05 control total ---
  ...
  for (const line of opexLines) {
    ...
    for (let m = 0; m < 12; m++) arr[m] += line.monthlyValues[m];
    groupTotals.set(key, arr);
  }
  const gateBResults: GateResult[] = [];
  ...
  if (gateBFailed.length) reportLines.push(gateBFailed.slice(0, 40).map((g) => `- INFO: ${g.detail}`).join('\n'));
  ...
  for (const line of opexLines) {
    ...
    if (series.length) gateCResults.push(...svc.gateDriverRecomputation(line, series));
  }
  ...
  if (gateCFailed.length) reportLines.push(gateCFailed.map((g) => `- FAIL: ${g.detail}`).join('\n'));
  reportLines.push('');
